...
def sparseLSTMCell(input, hidden, w_ih, w_hh, b_ih=None, b_hh=None, lambda_sh = None, D_sh = None, mu =None, D_1d = None):
    if input.is_cuda:   ##<------, D_sh=None, mu=None
    ...
    # hy_spa1 = []
    # for i in range(hy.size(0)):
    #     temp = D_sh[0:200, 0:200].matmul(F.tanh(hy[i][0:200] + mu[0:200]) + F.tanh(hy[i][0:200] - mu[0:200]))
    #     temp = temp.data.numpy()
    #     hy_spa1.append(temp)
    # hy_spa1 = Variable(torch.FloatTensor(np.array(hy_spa1)))
    # hy_spa2 = hy[:, 200:459]
    # hy_spa = torch.cat([hy_spa1, hy_spa2], 1)
    # D_sh[0:200, 0:200].matmul(

    hy_spa1 = []
    for i in range(hy.size(0)):
        temp = D_1d.mul(F.tanh(hy[i][0:200] + mu[0:200]) + F.tanh(hy[i][0:200] - mu[0:200]))
        hy_spa1.append(temp)  # 0.5*


    hy_spa1 = torch.stack(hy_spa1)
    # kkk =  torch.sum(hy_spa1**2)
    # print('kkk=',kkk)
    hy_spa2 = hy[:, 200:459]
    hy_spa = torch.cat([hy_spa1, hy_spa2], 1)

    # mu.register_hook(lambda g: print(g))
    # print('mu=', mu[3])
    # print('grad=', mu.grad)
    # print('w=', w_hh.grad)

    # lambda_sh0 = lambda_sh.data[0]
    # hy_spa1 = F.hardshrink(hy[:, 0:200], lambd=lambda_sh0)  #
    # hy_spa2 = hy[:, 200:459]
    # hy_spa = torch.cat([hy_spa1, hy_spa2], 1)
