...
class RNNBase(Module):
    ...
    def __init__(self, mode, input_size, hidden_size,
                 ...
                 dropout=0, bidirectional=False):  ##True
        ...
        for layer in range(num_layers):
            for direction in range(num_directions):
                ...
                D_sh = Parameter(torch.Tensor(hidden_size, hidden_size))
                mu = Parameter(torch.Tensor(hidden_size))
                lambda_sh = Parameter(torch.Tensor(1))                        # <----
                D_1d = Parameter(torch.Tensor(200))
                layer_params = (w_ih, w_hh, b_ih, b_hh, lambda_sh, D_sh, mu, D_1d)  # <----
                ...
                param_names += ['lambda_sh_l{}{}', 'D_l{}{}', 'mu_l{}{}', 'D1d_l{}{}']     # <----
    ...
    def flatten_parameters(self):
        ...
        from my2func2rnn import CudnnRNN
    ...
    def reset_parameters(self):
        ...
        #     elif index==4:
        #         weight.data = 2*torch.eye(hidden_size)
        #     elif index==5:
        #         weight.data = 0.5*torch.ones(hidden_size)
        #     elif index == 6:
        #         weight.data = 0.02*torch.ones(2)
        #     elif index<=10:
        #         weight.data.uniform_(-stdv, stdv)
        #     elif index==11:
        #         weight.data = 2*torch.eye(hidden_size)
        ...
        for weight in self.parameters():
            if index <= 1:
                ...
                weight.data = 0.01 * torch.ones(1)
            elif index <= 5:
                weight.data = 0.5*torch.eye(self.hidden_size)
            elif index <= 6:
                weight.data.fill_(2)
            elif index == 7:
...
class sparseLSTM(RNNBase):
    ...
    def __init__(self, *args, **kwargs):
        super(sparseLSTM, self).__init__('sparseLSTM', *args, **kwargs)
...
class sparseLSTMCell(RNNCellBase):
    ...
    def __init__(self, input_size, hidden_size, bias=True):
        ...
        self.lambda_sh = Parameter(torch.Tensor(1))
        if bias:
    ...
    def forward(self, input, hx):
        return my2func2rnn.sparseLSTMCell(
            input, hx,
            self.weight_ih, self.weight_hh,
            self.bias_ih, self.bias_hh, self.lambda_sh #, self.D_sh, self.mu
        )
